...
const CommentSection: React.FC<CommentProps> = ({ listingId, comments = [], currentUser
}) => {
  const {
    register,
    handleSubmit,
  ...
  const onSubmit: SubmitHandler<FieldValues> = (data) => {
    ...
    axios.post(`/api/comments/${listingId}`, data)
      .then(() => {
    ...
    <section className="bg-white  py-8 lg:py-16 antialiased">
      <div className="mx-auto px-4">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-lg lg:text-2xl font-bold text-gray-900">Reviews({comments.length})</h2>
        </div>
        {
          currentUser && <form className="mb-6" onSubmit={handleSubmit(onSubmit)}>
            ...
            <div className="flex items-center mb-4">
              ...
              {Array.from({ length: 5 }, (_, index) => (
                ...
                >
                  <svg
                    className={`w-4 h-4 ${index < (hoverRating || rating) ? 'text-rose-500' : 'text-hover'}`}
                    aria-hidden="true"
