...
const SearchModal = () => {
    ...
    const [stayOption, setStayOption] = useState<'Weekend' | 'Week' | 'Month'>('Weekend');
    ...
    useEffect(() => {
        ...
        for (let i = 0; i < 12; i++) {
            ...
            generatedMonths.push(date.toLocaleString('default', { month: 'long', year: 'numeric' }));
        }
    ...
    const handleSelectCat = (selectedCategory: string) => {
        ...
        if (!index) {
            ...
            const newCategories = [...category.filter(cat => cat !== selectedCategory)]
            setCategory(newCategories)
        }
    ...
    }, [step, searchModal, location, router, guestCount, roomCount, bathroomCount, dateRange, onNext, params, category]);
    const experienceLabels = ["1+ years", "3+ years", "5+ years", "7+ years", "9+ years"];
    ...
    if (step == STEPS.DATE) {
        bodyContent = (
            <div className="flex flex-col gap-8">
                ...
                />
                <div className={`flex self-center justify-center w-fit p-2 rounded-full bg-neutral-100 items-center gap-2 }`} >
                    <button className={`"font-semibold py-1 px-2 rounded-full hover:shadow-md  active:bg-white   ${calenderOption === 'Dates' && 'bg-white'}`} onClick={() => setCalenderOption("Dates")}>Dates</button>

                    <button className={`font-semibold py-1 px-2 rounded-full  hover:shadow-md  active:bg-white  ${calenderOption === 'Flexible' && 'bg-white'}`} onClick={() => setCalenderOption("Flexible")}>Flexible</button>
                </div>
                {
                    calenderOption === "Dates" ?
                        ...
                        (
                            <>
                                <h2 className="text-xl font-semibold text-center">Stay for a {stayOption}</h2>
                                ...
                                <div className="flex w-full items-center">
                                    <button onClick={() => scroll(-200)} type="button" className="text-black border shadow-sm  bg-white font-medium rounded-full text-sm p-2 h-fit text-center  items-center mr-4  hover:shadow-md">
                                    ...
                                    <div id="monthsContainer" className="flex  no-scrollbar overflow-x-auto gap-3 py-1">
                                        ...
                                        {months.map((month, index) => (
                                            <button
                                                ...
                                                ${selectedMonths.includes(month) ? 'border border-gray-700' : ''
                                                    }`}
                                    ...
                                    <button onClick={() => scroll(200)} type="button" className="text-black border shadow-sm  bg-white font-medium rounded-full text-sm p-2 h-fit text-center items-center ml-4  hover:shadow-md">
