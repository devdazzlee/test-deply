...
import { GiHuntingHorn, GiAerialSignal, GiLifeBar, GiFullMotorcycleHelmet, GiLovers, GiBoatFishing } from "react-icons/gi";
import { MdOutlineFitnessCenter, MdOutlineTravelExplore, MdRealEstateAgent, MdFamilyRestroom, MdProductionQuantityLimits, MdSportsHandball, MdEvent, MdOutlineCorporateFare } from "react-icons/md";
import { FaShoppingBag, FaJournalWhills, FaHeart, FaHatCowboy, FaWater } from "react-icons/fa";
        ...
        <Suspense>
            <Container>
                <div className="flex items-center">
                    ...
                    <button onClick={() => scroll(-300)} type="button" className="text-black border shadow-sm  bg-white hidden font-medium rounded-full text-sm p-2 h-fit text-center sm:inline-flex items-center mr-4  hover:shadow-md">
                    ...
                    <div id="categoriesContainer" className="pt-4 flex flex-row shadow-y-md items-center justify-between overflow-x-auto no-scrollbar">
                        {categories.map((item) => (
                            <CategoryBox
                                key={item.label}
                                label={item.label}
                    ...
                    <button onClick={() => scroll(300)} type="button" className="text-black border shadow-sm  bg-white font-medium rounded-full text-sm p-2 h-fit text-center hidden sm:inline-flex items-center ml-4  hover:shadow-md">
